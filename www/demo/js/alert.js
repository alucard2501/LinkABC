(function($) {        
    $.alerts = {         
        ...
        alertDelay: function(title, message, callback) {  
          if( title == null ) title = 'Alert';  
          $.alerts._show(title, message, null, 'alertDelay', function(result) {  
              if( callback ) callback(result);  
          });  
        },
        ...
        _show: function(title, msg, value, type, callback) {  
                    ...
                    if (type == "alertDelay") {  
                      //_html += '<input id="mb_btn_ok" disable type="button" value="即将跳转(3)" />';  
                      _html += '<input id="mb_btn_ok" type="button" value="我知道了" />';  
                    }
                    if (type == "confirm") {  
            ...
            switch( type ) {  
                ...
                case 'alertDelay':  
                    // setTimeout(function(){
                    //   $("#mb_btn_ok").val("即将跳转(2)");
                    //   setTimeout(function(){
                    //     $("#mb_btn_ok").val("即将跳转(1)");
                    //     setTimeout(function(){
                    //       $.alerts._hide();  
                    //       callback(true);  
                    //     },1000)
    ...
    myAlertDelay = function(title, message, callback) {  
      $.alerts.alertDelay(title, message, callback);  
    }  
